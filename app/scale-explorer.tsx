  ...
  useEffect(() => {
    ...
    if (isPlayingProgression && chordProgression.length > 0) {
      intervalId = setInterval(() => {
        setActiveProgressionIndex((activeProgressionIndex + 1) % chordProgression.length);
      }, progressionSpeedMs);
    }
    return () => clearInterval(intervalId);
  }, [isPlayingProgression, chordProgression, activeProgressionIndex, progressionSpeedMs]);
    ...
    <ScrollView style={styles.menuScroll}>
      <View style={styles.menuContent}>
        ...
        <View style={styles.toggleRow}>
          <Text style={[styles.controlLabel, { color: colors.text }]}>Show Scale Notes</Text>
          <Switch 
            value={showScale} 
            onValueChange={(v) => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setShowScale(v); }}
        ...
        <View style={styles.toggleRow}>
          <Text style={[styles.controlLabel, { color: colors.text }]}>Show Chord Tones</Text>
          <Switch 
            value={showChord} 
            onValueChange={(v) => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setShowChord(v); }}
            trackColor={{ false: colors.border, true: colors.tint }}
          />
        </View>

        {showChord && (
          <View style={[styles.progressionContainer, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.toggleRow}>
              <Text style={[styles.controlLabel, { color: colors.textSecondary }]}>Diatonic Only</Text>
              <Switch 
                value={hideNonScaleChordTones} 
                onValueChange={(v) => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setHideNonScaleChordTones(v); }}
                trackColor={{ false: colors.border, true: colors.tint }}
              />
            </View>
              ...
              <View style={styles.sequenceViewer}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.sequenceScroll}>
                  {chordProgression.map((chord, idx) => {
                    ...
                    const chordName = CHORDS.find(c => c.id === chord.suffix)?.name || chord.suffix;
                    return (
                      <Pressable 
                        key={`prog-${idx}`} 
                        style={[
                          ...
                          isActive && [styles.sequenceBadgeActive, { backgroundColor: colors.tint, borderColor: colors.tint }]
                        ]}
                        onPress={() => {
                          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                      ...
                      >
                        <Text style={[styles.sequenceText, { color: colors.text }, isActive && [styles.sequenceTextActive, { color: "#fff" }]]}>
                        ...
                        >
                          <Text style={[styles.sequenceRemoveText, { color: colors.textMuted }]}>✕</Text>
                        </Pressable>
                      </Pressable>
                    );
                  })}
                ...
                <View style={[styles.transportBar, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  <Pressable 
                    ...
                    onPress={() => {
                      ...
                      if (!isPlayingProgression) {
                        setActiveProgressionIndex(Math.max(0, activeProgressionIndex - 1));
                      }
                    }}
                  >
                    <Text style={[styles.transportIcon, { color: colors.text }]}>⏮</Text>
                  </Pressable>
                  <Pressable 
                    style={[styles.transportBtn, styles.transportPlayBtn, { backgroundColor: colors.surface }]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                      setIsPlayingProgression(!isPlayingProgression);
                    }}
                  >
                    <Text style={[styles.transportIcon, { color: colors.text }]}>{isPlayingProgression ? "⏸" : "▶️"}</Text>
                  </Pressable>
                  <Pressable 
                    style={styles.transportBtn}
                    onPress={() => {
                  ...
                  <View style={[styles.speedControls, { backgroundColor: colors.surface }]}>
                    <Pressable 
                      style={[styles.speedBtn, progressionSpeedMs === 3000 && [styles.speedBtnActive, { backgroundColor: colors.card }]]}
                      onPress={() => setProgressionSpeedMs(3000)}
                    >
                      <Text style={[styles.speedText, { color: colors.textSecondary }, progressionSpeedMs === 3000 && [styles.speedTextActive, { color: colors.tint }]]}>Slow</Text>
                    </Pressable>
                    <Pressable 
                      style={[styles.speedBtn, progressionSpeedMs === 2000 && [styles.speedBtnActive, { backgroundColor: colors.card }]]}
                      onPress={() => setProgressionSpeedMs(2000)}
                    >
                      <Text style={[styles.speedText, { color: colors.textSecondary }, progressionSpeedMs === 2000 && [styles.speedTextActive, { color: colors.tint }]]}>Med</Text>
                    </Pressable>
                    <Pressable 
                      style={[styles.speedBtn, progressionSpeedMs === 1000 && [styles.speedBtnActive, { backgroundColor: colors.card }]]}
                      onPress={() => setProgressionSpeedMs(1000)}
                    >
                      <Text style={[styles.speedText, { color: colors.textSecondary }, progressionSpeedMs === 1000 && [styles.speedTextActive, { color: colors.tint }]]}>Fast</Text>
                    </Pressable>
                  </View>
                </View>
              </View>
          ...
          <View style={styles.overlayContainer}>
            ...
            <View style={[styles.overlayMenu, { backgroundColor: colors.background }]}>
              <View style={[styles.overlayHeader, { borderBottomColor: colors.border }]}>
                <Text style={[styles.overlayTitle, { color: colors.text }]}>Fretboard Controls</Text>
